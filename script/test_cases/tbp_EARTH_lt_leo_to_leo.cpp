/**
	tbp_EARTH_lt_leo_to_leo.cpp

	Purpose: Low-thrust LEO to LEO transfer execution script.
...
SolverParameters get_SolverParameters_tbp_EARTH_lt_leo_to_leo(
	...
	vectordb huber_loss_coefficient_sequence{1e-2, 5e-3, 5e-3};
	double DDP_tol = 1e-4;
	double AUL_tol = 1e-6;
	double PN_tol = 1e-10;
	double PN_active_constraint_tol = 1e-11;
	unsigned int DDP_max_iter = 100;
	unsigned int AUL_max_iter = 30;
...
void tbp_EARTH_lt_leo_to_leo(int argc, char** argv) {
	...
	// Set dynamics
	Dynamics dynamics = get_tbp_EARTH_geo_lt_dynamics();
	...
	if (save_results) {
		...
		string file_name = "./data/datasets/tbp_EARTH_lt_leo_to_leo";
